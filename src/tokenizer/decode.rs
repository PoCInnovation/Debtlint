...
{
    let Some(entry) = vocabulary.get(token) else {
        // get the entry from the vocab
...
pub fn decode_sequence(sequence: &[Token], vocabulary: &Vocabulary) -> String {
    sequence
        .iter()
        .map(|&token| decode_token(token, vocabulary))
        .collect() // decode the sequence and return the string
}
